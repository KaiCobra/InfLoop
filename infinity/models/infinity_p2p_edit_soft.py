...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        # Blending 公式選擇：
        #   'none'        = 停用 soft blending，維持原始 hard replacement（預設）
        #   'linear'      = logits 空間線性混合：α * source_logits + (1-α) * target_logits
        #   'prob_linear' = 機率空間線性混合：α * source_one_hot + (1-α) * softmax(target_logits)
        #   'geometric'   = 幾何平均：exp(α * log(source_prob) + (1-α) * log(target_prob))
        #   'slerp'       = 球面插值（Spherical Linear Interpolation）
        soft_blend_temperature=10.0,
        # Source one-hot logits 的溫度。值越大 source 信號越硬（deterministic）；
        # 值越小 source 信號越柔和，更容易被 target logits 影響。
    ):   # returns List[idx_Bl]
        ...
        while si < len(scale_schedule):   # si: i-th segment
            ...
            # ── Soft Blending：在 sampling 前保存 logits ──
            _soft_blending_active = (
                soft_blend_method != 'none'
                and p2p_token_storage is not None
                and not p2p_save_tokens  # target gen 階段才 blend
                and p2p_token_storage.has_tokens_for_scale(si)
            )
            saved_logits_BlV = None

            if self.use_bit_label:
                tmp_bs, tmp_seq_len = logits_BlV.shape[:2]
                logits_BlV = logits_BlV.reshape(tmp_bs, -1, 2)
                if _soft_blending_active:
                    saved_logits_BlV = logits_BlV.clone()  # [B, seq_len*d, 2]
            ...
            if vae_type != 0:
                ...
                if p2p_token_storage is not None and p2p_token_storage.has_tokens_for_scale(si):
                    ...
                    if source_indices_loaded is not None:
                        ...
                        # ── 分支 B：scale >= N → Attention 空間遮罩引導 ──
                        # 遮罩由外部計算並事先存入 storage
                        # spatial_mask: [B, 1, h, w, 1]
                        #   - bool mask → hard replacement（原始行為）
                        #   - float mask → soft blending 的 α 權重
                            ...
                            if spatial_mask is not None:
                                ...
                                # ── Soft Blending 路徑 ──
                                if _soft_blending_active and saved_logits_BlV is not None:
                                    # α: replacement weight（1.0 = 100% source，0.0 = 100% target）
                                    if spatial_mask.dtype == torch.bool:
                                        alpha = spatial_mask.float()  # [B, 1, h, w, 1]
                                    else:
                                        alpha = spatial_mask.float().clamp(0.0, 1.0)

                                    # source one-hot logits: [B, 1, h, w, d, 2]
                                    src_bits = source_indices_loaded.long()  # [B, 1, h, w, d] values 0/1
                                    src_one_hot = F.one_hot(src_bits, num_classes=2).float()  # [B,1,h,w,d,2]
                                    src_logits = src_one_hot * soft_blend_temperature  # scale

                                    # target logits: [B, seq*d, 2] → [B, 1, h, w, d, 2]
                                    _, _, d_bits = src_bits.shape[2], src_bits.shape[3], src_bits.shape[4]
                                    tgt_logits = saved_logits_BlV.reshape(B, 1, h_cur, w_cur, d_bits, 2)

                                    # α 擴展到 [B, 1, h, w, 1, 1] 以 broadcast
                                    alpha_6d = alpha.unsqueeze(-1)  # [B, 1, h, w, 1, 1]

                                    # ── 選擇 blending 公式 ──
                                    if soft_blend_method == 'linear':
                                        # logits 空間線性混合
                                        blended = alpha_6d * src_logits + (1.0 - alpha_6d) * tgt_logits

                                    elif soft_blend_method == 'prob_linear':
                                        # 機率空間線性混合
                                        tgt_probs = F.softmax(tgt_logits, dim=-1)
                                        blended_probs = alpha_6d * src_one_hot + (1.0 - alpha_6d) * tgt_probs
                                        # 轉回 logits 空間（加 eps 防 log(0)）
                                        blended = torch.log(blended_probs + 1e-8)

                                    elif soft_blend_method == 'geometric':
                                        # 幾何平均（log-prob 空間加權平均）
                                        tgt_log_probs = F.log_softmax(tgt_logits, dim=-1)
                                        src_log_probs = torch.log(src_one_hot + 1e-8)
                                        blended = alpha_6d * src_log_probs + (1.0 - alpha_6d) * tgt_log_probs

                                    elif soft_blend_method == 'slerp':
                                        # 球面插值
                                        tgt_probs = F.softmax(tgt_logits, dim=-1)
                                        # 歸一化到單位球
                                        src_norm = src_one_hot / (src_one_hot.norm(dim=-1, keepdim=True) + 1e-8)
                                        tgt_norm = tgt_probs / (tgt_probs.norm(dim=-1, keepdim=True) + 1e-8)
                                        cos_omega = (src_norm * tgt_norm).sum(dim=-1, keepdim=True).clamp(-1, 1)
                                        omega = torch.acos(cos_omega)
                                        sin_omega = torch.sin(omega).clamp(min=1e-8)
                                        t = alpha_6d
                                        blended_probs = (torch.sin(t * omega) / sin_omega) * src_one_hot + \
                                                        (torch.sin((1 - t) * omega) / sin_omega) * tgt_probs
                                        blended = torch.log(blended_probs + 1e-8)

                                    else:
                                        raise ValueError(
                                            f"Unknown soft_blend_method: '{soft_blend_method}'. "
                                            f"Supported: 'linear', 'prob_linear', 'geometric', 'slerp'"
                                        )

                                    # 從 blended logits 重新取樣
                                    blended_flat = blended.reshape(B, -1, 2)  # [B, h*w*d, 2]
                                    resampled = sample_with_top_k_top_p_also_inplace_modifying_logits_(
                                        blended_flat, rng=rng,
                                        top_k=top_k or self.top_k,
                                        top_p=top_p or self.top_p,
                                        num_samples=1,
                                    )[:, :, 0]  # [B, h*w*d]
                                    idx_Bld = resampled.reshape(B, 1, h_cur, w_cur, -1)

                                    prob_mean = float(alpha.mean().item())
                                    print(
                                        f"[Soft-Blend] Scale {si}: "
                                        f"method={soft_blend_method}, "
                                        f"temp={soft_blend_temperature:.1f}, "
                                        f"α_mean={prob_mean:.3f}, "
                                        f"h={h_cur} w={w_cur}"
                                    )

                                # ── 原始 Hard Replacement 路徑 ──
                                else:
                                    if spatial_mask.dtype == torch.bool:
