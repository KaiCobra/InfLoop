"""
Definition of Infinity transformer model with Float-level (Indices-level) Rollback Merge.

KEY DIFFERENCE from infinity.py:
- infinity.py: Merges rollback scales at CODES level (binary ±0.1768 features)
- infinityFloat.py: Merges rollback scales at INDICES level (integer indices before quantization)

ADVANTAGES:
1. Avoids quantization artifacts from averaging binary values
2. Can use discrete operations (voting, XOR, etc.) on integer indices
3. More flexible merge strategies without worrying about binary constraints
4. Only converts to codes AFTER merging is complete

MERGE MODES (in _merge_prev_scale_indices):
- Mode 0: Direct replacement (use current scale only)
- Mode 1: Random token-wise selection with cfg_val probability
- Mode 2: Bit-wise voting (average and round)
- Mode 3: Weighted blend based on cfg_val
- Mode 4: Spatial checkerboard pattern mixing
- Mode 5: Bit-wise XOR mixing (experimental)
- Mode 6: CFG-style probabilistic replacement

USAGE:
Same as infinity.py, just import infinityFloat instead:
    from infinity.models.infinityFloat import Infinity
"""
...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        save_img_path=None,
        sampling_per_bits=1,
        rollback_schedule=None,  # List of dicts: [{"scale": 4, "rollback_to": 2, "times": 3}]
                                   # means: after generating scale 4, rollback to scale 2, re-run scales 2→4, repeat 3 times
                                   # Also accepts legacy dict format: {4: 1} (rollback_to defaults to scale-1)
        ...
        # Normalize rollback_schedule to new list-of-dicts format
        # New format: [{"scale": 4, "rollback_to": 2, "times": 3}]
        # Legacy format: {4: 1} => [{"scale": 4, "rollback_to": 3, "times": 1}]
        if rollback_schedule is None:
            rollback_rules = []
        elif isinstance(rollback_schedule, dict):
            # Legacy format: {scale_idx: num_retries}, rollback_to = scale - 1
            rollback_rules = [
                {"scale": s, "rollback_to": s - 1, "times": t}
                for s, t in rollback_schedule.items()
            ]
        elif isinstance(rollback_schedule, list):
            rollback_rules = rollback_schedule
        else:
            rollback_rules = []
        
        # Build lookup: for each trigger scale, store (rollback_to, times)
        rollback_lookup = {}  # {trigger_scale: {"rollback_to": int, "times": int}}
        for rule in rollback_rules:
            s = rule["scale"]
            rb_to = rule.get("rollback_to", s - 1)
            times = rule.get("times", 1)
            assert 0 <= rb_to < s, f"rollback_to ({rb_to}) must be >= 0 and < scale ({s})"
            rollback_lookup[s] = {"rollback_to": rb_to, "times": times}
        
        # Determine which scales need state saving (any scale that is a rollback target)
        scales_needing_state_save = set()
        for rule_info in rollback_lookup.values():
            scales_needing_state_save.add(rule_info["rollback_to"])
        
        scale_states = {}  # {scale_idx: saved state dict}
        rollback_counter = {}  # {trigger_scale: current_retry_count}
        ...
        while si < len(scale_schedule):   # si: i-th segment
            ...
            # Save state if this scale is a rollback target (any rule's rollback_to)
            if si in scales_needing_state_save and si not in scale_states:
                def _save_state():
                    return {
                        ...
                        'kv_cache_len': cur_L,  # cumulative token count BEFORE this scale
                    }
                scale_states[si] = _save_state()
                print(f"[Rollback] Saving state at scale {si} (cur_L={cur_L})")
            
            # Determine if rollback should happen after this scale
            if si in rollback_lookup:
                rule = rollback_lookup[si]
                if rollback_counter.get(si, 0) < rule["times"]:
                    should_rollback = True
                    if si not in rollback_counter:
            ...
            if should_rollback:
                rule = rollback_lookup[si]
                rb_to = rule["rollback_to"]
                rollback_counter[si] += 1
                print(f"[Rollback] Scale {si} completed. Rolling back to scale {rb_to} "
                      f"(retry {rollback_counter[si]}/{rule['times']})")
                
                # Restore state from the rollback target scale
                if rb_to in scale_states:
                    ...
                    if rollback_merge_mode is not None and si > 0 and vae_type != 0 and vae is not None:
                        saved_state_for_merge = scale_states[rb_to]
                        
                        # Get previous scale indices (from the rollback target)
                        if isinstance(saved_state_for_merge.get('idx_Bld_list', None), list) and len(saved_state_for_merge['idx_Bld_list']) > 0:
                            prev_idx = saved_state_for_merge['idx_Bld_list'][-1]  # last idx at rollback target
                            cur_idx = idx_Bld  # Current (trigger) scale indices [B, 1, h_cur, w_cur, d]
                            ...
                            # Build last_stage from merged codes for the rollback target's next scale
                            next_scale_after_rb = rb_to  # we will re-run starting from rb_to
                            if next_scale_after_rb < len(vae_scale_schedule):
                                merged_full = F.interpolate(merged_codes, size=vae_scale_schedule[-1], mode=vae.quantizer.z_interplote_up)
                                merged_next = F.interpolate(merged_full, size=vae_scale_schedule[next_scale_after_rb], mode=vae.quantizer.z_interplote_up)
                                ...
                                # Update the saved state's last_stage with merged result
                                scale_states[rb_to]['last_stage'] = merged_next

                    saved_state = scale_states[rb_to]
                    ...
                    # Truncate KV cache to the rollback target's cumulative length
                    kv_trunc_len = saved_state['kv_cache_len']
                    if inference_mode:
                        for b in self.unregistered_blocks:
                            attn = b.sa if isinstance(b, CrossAttnBlock) else b.attn
                            if attn.cached_k is not None:
                                L_dim = 1 if attn.using_flash else 2
                                attn.cached_k = attn.cached_k.narrow(L_dim, 0, kv_trunc_len) if kv_trunc_len > 0 else None
                                attn.cached_v = attn.cached_v.narrow(L_dim, 0, kv_trunc_len) if kv_trunc_len > 0 else None
                    else:
                        for block_chunk_ in self.block_chunks:
                            for module in block_chunk_.module.module:
                                attn = module.sa if isinstance(module, CrossAttnBlock) else module.attn
                                if attn.cached_k is not None:
                                    L_dim = 1 if attn.using_flash else 2
                                    attn.cached_k = attn.cached_k.narrow(L_dim, 0, kv_trunc_len) if kv_trunc_len > 0 else None
                                    attn.cached_v = attn.cached_v.narrow(L_dim, 0, kv_trunc_len) if kv_trunc_len > 0 else None
                    
                    print(f"[Rollback] State restored to scale {rb_to} (cur_L={cur_L}). "
                          f"Re-running scales {rb_to}→{si}...")
                    # Jump back to rollback target scale to re-run from there
                    si = rb_to
                    continue
                else:
                    print(f"[Rollback] WARNING: No saved state for scale {rb_to}, skipping rollback.")
