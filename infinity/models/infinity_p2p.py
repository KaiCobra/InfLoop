"""
Definition of Infinity transformer model with Prompt-to-Prompt (P2P) Image Editing.

KEY FEATURES:
- Based on infinityFloat.py (indices-level rollback merge)
- Adds P2P editing capability: save bitwise tokens from source prompt generation
- Apply source tokens to target prompt generation with probabilistic replacement
- Support for future mask-guided selective token replacement

P2P WORKFLOW:
1. Generate source image, save bitwise tokens (±0.1768) from first N scales
2. Generate target image, replace target tokens with source tokens (probabistically)
3. (Future) Use mask to control which regions get source tokens

TOKEN REPLACEMENT MODES:
- prob_replacement: Replace each token independently with given probability
- mask_guided: (Future) Use mask [0,1] to control replacement per spatial position

ADVANTAGES:
1. Preserves object structure/appearance from source image
2. Avoids quantization artifacts by working at indices level
3. Flexible spatial control via masks (extensible interface)
4. Enables controlled semantic editing while maintaining layout

USAGE:
    from infinity.models.infinity_p2p import Infinity
    # See run_p2p.py for full workflow example
"""
...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        # P2P (Prompt-to-Prompt) editing parameters
        p2p_token_storage=None,  # BitwiseTokenStorage object containing source tokens
        p2p_token_replace_prob=0.5,  # Probability of replacing target tokens with source tokens
        p2p_use_mask=False,  # Whether to use mask for selective token replacement
        p2p_save_tokens=True,  # Whether to save tokens into storage (disable during target generation)
    ):   # returns List[idx_Bl]
        ...
        while si < len(scale_schedule):   # si: i-th segment
            ...
            if vae_type != 0:
                ...
                # ===== P2P (Prompt-to-Prompt) Token Management =====
                # Step 1: Save indices if p2p_token_storage is provided (SOURCE image generation)
                if p2p_token_storage is not None and p2p_save_tokens:
                    # Save indices directly (not codes)
                    # idx_Bld shape: [B, 1, h, w, d] - already in correct format
                    p2p_token_storage.save_tokens(si, idx_Bld.clone())
                
                # Step 2: Apply token replacement if p2p_token_storage has tokens (TARGET image generation)
                if p2p_token_storage is not None and p2p_token_storage.has_tokens_for_scale(si):
                    # Load source indices for this scale
                    source_indices_loaded = p2p_token_storage.load_tokens(si, idx_Bld.device)
                    
                    if source_indices_loaded is not None:
                        # source_indices_loaded format: [B, 1, h_src, w_src, d]
                        B_src, _, h_src, w_src, d_src = source_indices_loaded.shape
                        _, _, h_cur, w_cur, _ = idx_Bld.shape
                        
                        # Interpolate spatial dimensions if needed
                        if (h_src, w_src) != (h_cur, w_cur):
                            # Reshape to [B*1, d, h_src, w_src] for interpolation
                            source_indices_for_interp = source_indices_loaded.squeeze(1).permute(0, 3, 1, 2).float()
                            source_indices_for_interp = F.interpolate(
                                source_indices_for_interp,
                                size=(h_cur, w_cur),
                                mode='nearest'
                            )
                            # Reshape back to [B, 1, h_cur, w_cur, d]
                            source_indices_loaded = source_indices_for_interp.permute(0, 2, 3, 1).unsqueeze(1).long()
                        
                        # Apply probabilistic replacement
                        if p2p_token_replace_prob > 0:
                            mask = torch.rand(B, 1, h_cur, w_cur, 1, device=idx_Bld.device) < p2p_token_replace_prob
                            
                            # Apply mask-based replacement if mask available
                            if p2p_use_mask and p2p_token_storage.has_mask_for_scale(si):
                                spatial_mask = p2p_token_storage.load_mask(si, idx_Bld.device)
                                if spatial_mask is not None:
                                    # spatial_mask: [B, 1, h, w, 1] with values in [0, 1]
                                    mask = mask * spatial_mask  # Element-wise multiplication
                            
                            # Replace tokens where mask is True
                            # Broadcast source_indices to match idx_Bld shape if needed
                            if source_indices_loaded.shape[0] == 1 and B > 1:
                                source_indices_loaded = source_indices_loaded.expand(B, -1, -1, -1, -1)
                            
                            # Debug: log replacement info for stored scales
                            if hasattr(p2p_token_storage, 'num_scales') and si < p2p_token_storage.num_scales:
                                num_replaced = mask.sum().item()
                                total = mask.numel()
                                print(f"[P2P DEBUG] Scale {si}: Replacing {num_replaced}/{total} tokens ({100*num_replaced/total:.1f}%) with prob={p2p_token_replace_prob}")
                            
                            idx_Bld = torch.where(mask, source_indices_loaded, idx_Bld)
                        else:
                            # If replace_prob is 0, no replacement
                            pass
