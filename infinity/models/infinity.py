...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        def _merge_prev_scale(prev_feat: torch.Tensor, down_feat: torch.Tensor, mode: int, cfg_val: float = 1.0) -> torch.Tensor:
            if mode == 0:
                return down_feat
            if mode == 1:
                # Average and quantize back to binary values
                result = 0.5 * (prev_feat + down_feat)
                return torch.where(result >= 0.1, torch.tensor(0.1768, device=result.device, dtype=result.dtype), 
                                 torch.tensor(-0.1768, device=result.device, dtype=result.dtype))
            if mode == 2:
                # Geometric mean and quantize back to binary values
                result = torch.sqrt(torch.clamp(prev_feat * down_feat, min=0.0))
                return torch.where(result >= result.mean(), torch.tensor(0.1768, device=result.device, dtype=result.dtype), 
                                 torch.tensor(-0.1768, device=result.device, dtype=result.dtype))
            if mode == 3:
                # CFG-style weighted sum and quantize back to binary values
                result = (1 - cfg_val) * down_feat + cfg_val * prev_feat
                return torch.where(result >= 0.0, torch.tensor(0.1768, device=result.device, dtype=result.dtype), 
                                 torch.tensor(-0.1768, device=result.device, dtype=result.dtype))
            if mode == 4:
                # Geometric weighted mean and quantize back to binary values
                result = torch.sqrt(torch.clamp((down_feat ** (1 - cfg_val)) * (prev_feat ** cfg_val), min=0.0))
                return torch.where(result >= result.mean(), torch.tensor(0.1768, device=result.device, dtype=result.dtype), 
                                 torch.tensor(-0.1768, device=result.device, dtype=result.dtype))
            if mode == 5:
                # Nonlinear mixing based on feature sign agreement
                # When both features have same sign, blend them; otherwise use down_feat more
                sign_agreement = (prev_feat * down_feat > 0).float()  # 1 if same sign, 0 if different
                # Nonlinear weighting: stronger agreement -> more mixing
                mix_weight = sign_agreement * cfg_val
                result = mix_weight * prev_feat + (1 - mix_weight) * down_feat
                return torch.where(result >= 0.0, torch.tensor(0.1768, device=result.device, dtype=result.dtype), 
                                 torch.tensor(-0.1768, device=result.device, dtype=result.dtype))
            if mode == 6:
                # Random token-wise replacement with probability cfg_val
                # down_feat shape: [B, d, 1, h, w], values are binary: 0.1768 or -0.1768
                # Generate random mask at spatial token level (h, w)
                B, d, _, h, w = down_feat.shape
                # Create random mask for each token position (h, w)
                mask = torch.rand(B, 1, 1, h, w, device=down_feat.device) < 0.5  # [B, 1, 1, h, w]
                # Apply mask: use down_feat where mask=True, prev_feat where mask=False
                return torch.where(mask, down_feat, prev_feat)
            return down_feat
        ...
        while si < len(scale_schedule):   # si: i-th segment
            ...
            if should_rollback:
                ...
                if si in scale_states:
                    # Optional: merge current scale result into previous scale state before regenerating
                    if rollback_merge_mode is not None and si > 0 and vae_type != 0 and cur_scale_codes is not None and vae is not None:
                        ...
                        if isinstance(saved_state_for_merge.get('idx_Bld_list', None), list) and len(saved_state_for_merge['idx_Bld_list']) > si - 1:
                            prev_idx = saved_state_for_merge['idx_Bld_list'][si - 1]
                            prev_codes = vae.quantizer.lfq.indices_to_codes(prev_idx, label_type='bit_label')

                        down_codes = F.interpolate(cur_scale_codes, size=vae_scale_schedule[si - 1], mode=vae.quantizer.z_interplote_up)
                        merged_codes = _merge_prev_scale(prev_codes, down_codes, rollback_merge_mode, cfg_val=cfg_list[si]) if prev_codes is not None else down_codes

                        # Build last_stage for scale si from merged previous-scale codes
