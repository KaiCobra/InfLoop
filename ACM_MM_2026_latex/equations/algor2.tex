...
\Require Source image $I_{\mathrm{src}}$, prompts $P_s, P_t$, focus word sets $\mathcal{F}_s, \mathcal{F}_t$,
         structural scales $N$, total scales $S$, optional reference mask $M_{\mathrm{ref}}$
...
\For{$k = 1$ \textbf{to} $S$}
    ...
    \If{$k \ge N$}
        ...
        \State $\tau_k^{(s)} \gets \mathrm{Threshold}(\hat{a}_k^{(s)},\, M_{\mathrm{ref}})$ \Comment{binary search or percentile}
    ...
    \If{$k \ge N$}
        ...
        \State $\tau_k^{(t)} \gets \mathrm{Threshold}(\hat{a}_k^{(t)},\, M_{\mathrm{ref}})$
