...
framework. Critically for our method, BSQ encoding is \textit{lossless}: encoding 
a real image into its bitwise token representation and decoding it back introduces 
no reconstruction distortion beyond the tokenizer's inherent capacity, a property 
we directly exploit when transplanting source image tokens into unedited regions 
to guarantee pixel-faithful background preservation by construction.
...
The overall architecture of our proposed method is illustrated in Figure~\ref{fig:pipeline}. The model first resolves spatial editing semantics via cross-modal attention analysis, then selectively rewrites only the targeted tokens. Concretely, our method performs a \textbf{dual-stream mask extraction} (Phases~1--2) followed by \textbf{mask-guided token composition} (Phase~3): \textbf{(1)~Source-Stream Mask Extraction} generates visual tokens conditioned on $P_s$ and extracts focus and preserve masks from cross-attention; \textbf{(2)~Target-Stream Mask Extraction} generates tokens conditioned on $P_t$ with background anchoring and extracts the target focus mask; and \textbf{(3)~Mask-Guided Token Composition} selectively replaces background tokens to produce the edited output. The edited token map at each scale is:
\begin{equation}
    r_k^e = M_k^{\mathrm{rep}} \odot r_k^s + (1 - M_k^{\mathrm{rep}}) \odot g_k
\end{equation}
where $g_k$ denotes tokens generated under the target prompt, $r_k^s$ the source tokens, and $\odot$ element-wise multiplication. The final edited image $I_e$ is decoded from $\{r_k^e\}_{k=1}^{K}$ by the BSQ-VAE decoder.
...
The filtered map $\hat{a}_k$ is binarized into two complementary masks using a single percentile parameter $p$ (default $p{=}0.8$). Let $\tau_k$ be the $p$-th percentile of $\hat{a}_k$ and $\tau_k^{\mathrm{low}}$ the $(1{-}p)$-th percentile; both thresholds are inherently scale-adaptive. When an external reference mask is available, we apply a binary search procedure to find the optimal $\tau_k$ automatically (see Algorithm~1 in the supplementary material). The resulting masks are:
\EqMasks
