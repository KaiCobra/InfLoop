...
from attention_map.extractor import CrossAttentionExtractor, AttentionCacheInjector, SelfAttentionExtractor
from tools.run_pie_edit_selfattn_seg import (
    extract_cross_attn_map,
    extract_intra_scale_self_attn,
    diffsegmenter_refine,
    save_heatmap,
    save_binary_mask,
    save_overlay,
)
...
# ============================================================
# load_transformer_soft — 載入 soft-blending 版本的 Infinity 模型
# ============================================================

def load_transformer_soft(vae, args):
    """Load transformer using the soft-blending Infinity model class."""
    import infinity.models.infinity_p2p_edit_soft as soft_module
    import tools.run_p2p_edit as base_module

    # 暫時替換 base_module 中的 Infinity class，讓 load_transformer 使用 soft 版本
    original_infinity = base_module.Infinity
    base_module.Infinity = soft_module.Infinity
    try:
        model = load_transformer(vae, args)
    finally:
        base_module.Infinity = original_infinity
    return model


# ============================================================
# gen_one_img_soft — gen_one_img 的擴展版，支援 soft blending
# ============================================================

def gen_one_img_soft(
    infinity_test,
    vae,
    text_tokenizer,
    text_encoder,
    ...
    # ── Soft Blending 新增參數 ──
    soft_blend_method='none',
    soft_blend_temperature=10.0,
):
    """
    gen_one_img 的擴展版本，新增 soft blending 參數。

    新增參數：
        soft_blend_method (str): blending 公式
            'none' / 'linear' / 'prob_linear' / 'geometric' / 'slerp'
        soft_blend_temperature (float): source one-hot logits 的溫度
    """
    sstt = time.time()
...
def run_one_case(
    ...
    # ── SA×CA DiffSegmenter: optional self-attention extractor ──
    sa_refine_enabled = bool(getattr(args, 'sa_refine_iterations', 0))
    sa_self_extractor: Optional[SelfAttentionExtractor] = None
    if sa_refine_enabled and has_source_focus:
        ...
        sa_max_scale = int(getattr(args, 'sa_max_scale', -1))
        sa_capture_end = sa_max_scale if sa_max_scale >= 0 else -1

        sa_self_extractor = SelfAttentionExtractor(
            model=infinity,
            block_indices=sa_block_indices,
            batch_idx=int(args.attn_batch_idx),
            aggregate_method="mean",
            capture_attention=True,
            capture_scale_start=args.num_full_replace_scales,
            capture_scale_end=sa_capture_end,
            capture_kv=False,
            inject_kv=False,
        )
        sa_self_extractor.register_patches()
    ...
    if sa_self_extractor is not None:
        sa_self_extractor.remove_patches()
    ...
    # ─────────────────────────────────────
    # Phase 1.4：DiffSegmenter SA×CA iterative refinement（視覺化用）
    # 使用 source focus words 的 CA map + intra-scale SA affinity matrix
    # 迭代精煉後存出每個 scale 的 heatmap 與 binary mask
    # ─────────────────────────────────────
    if sa_refine_enabled and sa_self_extractor is not None and has_source_focus:
        sa_iterations = int(getattr(args, 'sa_refine_iterations', 3))
        sa_threshold_pct = float(args.attn_threshold_percentile)
        num_full_replace = int(args.num_full_replace_scales)
        _sa_max_scale = int(getattr(args, 'sa_max_scale', -1))
        _sa_block_start = int(getattr(args, 'sa_block_start', 0))
        _sa_block_end = int(getattr(args, 'sa_block_end', -1))
        _depth = len(infinity.unregistered_blocks)
        _sa_block_start = max(0, _sa_block_start)
        _sa_block_end = (_depth - 1) if _sa_block_end < 0 else min(_sa_block_end, _depth - 1)
        _sa_block_indices = list(range(_sa_block_start, _sa_block_end + 1))

        seg_vis_dir = os.path.join(save_dir, 'selfattn_seg')
        sa_max_label = "all" if _sa_max_scale < 0 else str(_sa_max_scale)
        print(f"    [Phase 1.4] DiffSegmenter SA×CA refinement "
              f"({sa_iterations} iters, threshold={sa_threshold_pct}%, "
              f"SA blocks {_sa_block_indices[0]}-{_sa_block_indices[-1]}, "
              f"sa_max_scale={sa_max_label})")

        last_refined_mask: Optional[np.ndarray] = None
        for si in range(num_full_replace, total_scales):
            _, H, W = scale_schedule[si]

            ca_map = extract_cross_attn_map(
                source_extractor, attn_block_indices, si,
                source_focus_token_indices, (H, W),
            )
            if ca_map is None:
                continue

            beyond_sa_range = (_sa_max_scale >= 0 and si > _sa_max_scale)
            sa_matrix = None
            if not beyond_sa_range:
                sa_matrix = extract_intra_scale_self_attn(
                    sa_self_extractor, _sa_block_indices, si, (H, W),
                )
            ...
            if sa_matrix is not None:
                ...
                for iter_idx, iter_map in enumerate(iteration_results):
                    ...
                    save_heatmap(
                        iter_map,
                        os.path.join(seg_vis_dir, f'scale{si:02d}_{label}.png'),
                    )
                final_map = iteration_results[-1]
                thr = np.percentile(final_map, sa_threshold_pct)
                mask = (final_map >= thr)
                last_refined_mask = mask
                if H * W <= 1024:
                    save_heatmap(
                        sa_matrix,
                        os.path.join(seg_vis_dir, f'scale{si:02d}_sa_matrix.png'),
                    )
                ...
                mask = upsampled
                save_heatmap(ca_map, os.path.join(seg_vis_dir, f'scale{si:02d}_ca_raw.png'))
                print(f"      scale {si} ({H}x{W}): obj={mask.mean()*100:.1f}% [upsampled]")

            else:
                save_heatmap(ca_map, os.path.join(seg_vis_dir, f'scale{si:02d}_ca_raw.png'))
                thr = np.percentile(ca_map, sa_threshold_pct)
                mask = (ca_map >= thr)
                print(f"      scale {si} ({H}x{W}): obj={mask.mean()*100:.1f}% [CA only]")

            save_binary_mask(mask, os.path.join(seg_vis_dir, f'scale{si:02d}_mask_object.png'))
            save_overlay(img_np, mask,
                         os.path.join(seg_vis_dir, f'scale{si:02d}_overlay.png'))

        # Save SA×CA metadata
        os.makedirs(seg_vis_dir, exist_ok=True)
        with open(os.path.join(seg_vis_dir, 'meta.json'), 'w', encoding='utf-8') as f:
            json.dump({
                "source_prompt": source_prompt,
                "focus_words": source_focus_words_list,
                "focus_token_indices": source_focus_token_indices,
                "sa_refine_iterations": sa_iterations,
                "sa_max_scale": _sa_max_scale,
                "threshold_percentile": sa_threshold_pct,
                "ca_block_range": [attn_block_indices[0], attn_block_indices[-1]],
                "sa_block_range": [_sa_block_indices[0], _sa_block_indices[-1]],
                "num_full_replace_scales": num_full_replace,
            }, f, ensure_ascii=False, indent=2)
        print(f"    [Phase 1.4] SA×CA masks saved to {seg_vis_dir}")

        # cleanup SA extractor data
        del sa_self_extractor
    ...
    with autocast(dtype=torch.bfloat16):
        with torch.no_grad():
            target_image = gen_one_img_soft(
                ...
                inject_image_features=None,
                inject_schedule=None,
                soft_blend_method=getattr(args, 'soft_blend_method', 'none'),
                soft_blend_temperature=getattr(args, 'soft_blend_temperature', 10.0),
            )
...
def main() -> None:
    ...
    # ── Soft Blending 參數 ──
    parser.add_argument('--soft_blend_method', type=str, default='none',
                        choices=['none', 'linear', 'prob_linear', 'geometric', 'slerp'],
                        help='Soft blending 公式選擇。'
                             'none=停用（hard replacement），linear=logits 線性混合，'
                             'prob_linear=機率空間線性混合，geometric=幾何平均，slerp=球面插值')
    parser.add_argument('--soft_blend_temperature', type=float, default=10.0,
                        help='Source one-hot logits 的溫度。值越大 source 信號越硬（deterministic）。預設：10.0')
    # ── SA×CA DiffSegmenter 參數 ──
    parser.add_argument('--sa_refine_iterations', type=int, default=0,
                        help='DiffSegmenter SA×CA 迭代精煉次數（0=停用）。預設：0')
    parser.add_argument('--sa_block_start', type=int, default=0,
                        help='Self-attention block 起始 index。預設：0')
    parser.add_argument('--sa_block_end', type=int, default=-1,
                        help='Self-attention block 結束 index（-1=最後一個）。預設：-1')
    parser.add_argument('--sa_max_scale', type=int, default=-1,
                        help='SA 擷取的最大 scale index（-1=全部）。後期 scale OOM 時可限制。預設：-1')
