...
def gen_one_img(
    ...
    rollback_schedule=None,
    rollback_merge_mode=None,
):
    sstt = time.time()
    ...
    img = img_list[0]
    # Clean up intermediate results to free GPU memory
    del img_list
    return img
...
def add_common_arguments(parser):
    ...
    parser.add_argument('--rollback_merge_mode', type=int, default=-1, choices=[-1,0,1,2,3,4,5,6])
    parser.add_argument('--rollback_schedule', type=str, default='',
                        help='JSON string: list of rollback rules. '
                             'Example: \'[{"scale":4,"rollback_to":2,"times":3}]\' '
                             'means scale 4 rolls back to scale 2, repeated 3 times. '
                             'Multiple rules: \'[{"scale":4,"rollback_to":2,"times":3},{"scale":6,"rollback_to":4,"times":2}]\'')
...
if __name__ == '__main__':
    ...
    # Parse rollback_schedule from JSON string
    # New format: [{"scale": 4, "rollback_to": 2, "times": 3}, ...]
    rollback_schedule = None
    if args.rollback_schedule:
        rollback_schedule = json.loads(args.rollback_schedule)
        print(f'\nRollback schedule: {rollback_schedule}')
        for rule in rollback_schedule:
            print(f'  Scale {rule["scale"]} → rollback to scale {rule["rollback_to"]}, {rule["times"]} time(s)')
        print()

    # Step 1: Generate base image (no rollback)
    print('\n' + '='*80)
    print('STEP 1: Generating base image (no rollback)')
    print('='*80)
    ...
    # Ensure save directory exists and fix path if needed
    save_dir = args.save_file
    if save_dir.startswith('/') and not save_dir.startswith(os.path.expanduser('~')):
        save_dir = '.' + save_dir
        print(f'Warning: Changed save path from {args.save_file} to {save_dir} (permission issue)')
    ...
    # Step 2: Generate rollback images
    if rollback_schedule:
        print('\n' + '='*80)
        print('STEP 2: Generating rollback images')
        print('='*80)
        with autocast(dtype=torch.bfloat16):
            with torch.no_grad():
                generated_image = gen_one_img(
        ...
        # Build descriptive filename from schedule
        schedule_desc = '_'.join(
            f's{r["scale"]}rb{r["rollback_to"]}x{r["times"]}'
            for r in rollback_schedule
        )
        save_path = os.path.join(save_dir, f"output_rollback_{schedule_desc}.jpg")
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        img_np = generated_image.cpu().numpy()
        if img_np.dtype != np.uint8:
            img_np = np.clip(img_np, 0, 255).astype(np.uint8)
        cv2.imwrite(save_path, img_np)
        print(f'[✓] Rollback image saved to {save_path}')
    ...
    print('\n' + '='*80)
    print('LOOP ROLLBACK PIPELINE COMPLETE')
    print('='*80)
