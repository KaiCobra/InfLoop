...
需求對齊：
1. 讀取 extracted_pie_bench/{category}/{task}/meta.json 的 source/target prompt。
2. 先將兩個 prompt 的 [] 標記移除（保留括號內文字）。
3. 從 source/target 差異詞彙建立 focus words：
   - source_focus_words = " ".join(source_diff_words)
   - target_focus_words = " ".join(target_diff_words)
4. 每個 task 使用 image.jpg 作為 source_image，沿用 run_p2p_edit 參數流程。
5. 批次模式下模型僅初始化一次，避免重複 load model。
"""
...
from tools.run_pie_edit_soft import (  # noqa: E402
    run_one_case,
    load_transformer_soft,
)
...
def main() -> None:
    ...
    parser.add_argument("--soft_blend_method", type=str, default="none",
                        ...
                        help="Soft blending 公式選擇（none=hard replacement，linear=logits 線性混合）")
    parser.add_argument("--soft_blend_temperature", type=float, default=10.0,
                        help="Source one-hot logits 的溫度。值越大 source 信號越硬。預設：10.0")
    ...
    parser.add_argument("--sa_max_scale", type=int, default=-1,
                        help="SA 擷取的最大 scale index（-1=全部）。預設：-1")
    ...
    infinity = load_transformer_soft(vae, args)
