...
def compute_attention_mask_for_scale(
    ...
    low_attn: bool = False,
    sam_weight_map: Optional[np.ndarray] = None,
    sam_boost: float = 2.0,
) -> Optional[np.ndarray]:
    """
    計算指定 scale 的 attention-based 空間遮罩（支援 SAM3 mask 加權融合）。
    ...
    流程：
        ...
        4. （若有 SAM3 mask）對 attention map 進行加權放大
        5. 使用百分位數閾值二值化
    ...
    Args:
        ...
        sam_weight_map: (H_orig, W_orig) float ndarray，SAM3 mask（1.0=偵測區域，0.0=背景）。
                        會自動 resize 到 (spatial_h, spatial_w)。
        sam_boost: SAM3 加權強度。overlap 區域的 attention 乘以 (1 + sam_boost)。
    ...
    # SAM3 mask 加權：overlap 區域的 attention 被放大
    if sam_weight_map is not None:
        sam_resized = cv2.resize(
            sam_weight_map.astype(np.float32),
            (spatial_w, spatial_h),
            interpolation=cv2.INTER_LINEAR,
        )  # (spatial_h, spatial_w)
        # attention *= (1 + sam_boost * sam_mask)
        # sam_mask=1 的位置 → attention 乘以 (1+sam_boost)
        # sam_mask=0 的位置 → attention 不變
        filtered_attn = filtered_attn * (1.0 + sam_boost * sam_resized)
        sam_coverage = (sam_resized > 0.5).mean() * 100
        print(
            f"  Scale {scale_idx}: SAM3 加權 (boost={sam_boost:.1f})，"
            f"SAM 覆蓋 {sam_coverage:.1f}% 的空間位置"
        )
...
def collect_attention_text_masks(
    ...
    """
    收集各 scale 的 attention mask，不存入 storage（支援 SAM3 加權融合）。

    Args:
        low_attn: False（預設）= 高 attention focus mask（True=focus 不替換）
                  True = 低 attention preserve mask（True=極背景 強制保留 source token）
        sam_weight_map: (H_orig, W_orig) float ndarray，SAM3 mask。None = 不加權。
        sam_boost: SAM3 加權強度。
    ...
    sam_label = f" + SAM3 boost={sam_boost}" if sam_weight_map is not None else ""
    print(f"\n[Attention 遮罩計算 – {label} ({mode_label}{sam_label})] scale {num_full_replace_scales} ~ {len(scale_schedule)-1}")
...
if __name__ == '__main__':
    ...
    # ── SAM3 Mask 融合參數 ──
    parser.add_argument('--sam_mask', type=str, default=None,
                        help='SAM3 segmentation mask 圖片路徑（灰階/二值 PNG/JPG）。'
                             '白色=偵測區域（要編輯），黑色=背景。留空=不使用 SAM3 融合。')
    parser.add_argument('--sam_boost', type=float, default=2.0,
                        help='SAM3 加權強度。overlap 區域的 attention *= (1 + sam_boost)。預設：2.0')
    ...
    parser.add_argument('--inject_weights', type=str, default='',
                        help='Phase 1 各 scale 注入強度（空格分隔，長度=scale 總數）。'
                             '0.0=100%% source，1.0=自由生成。'
                             '若不指定，由 image_injection_scales 自動生成')
    parser.add_argument('--phase2_inject_weights', type=str, default='',
                        help='Phase 2 各 scale 注入強度（空格分隔，長度=scale 總數）。'
                             '0.5=50%% source + 50%% target。留空=不注入（原始行為）')
    ...
    print(f"SAM3 mask          : {args.sam_mask or '（未提供）'}")
    print(f"SAM3 boost         : {args.sam_boost}")
    ...
    if args.sam_mask is not None:
        sam_img = cv2.imread(args.sam_mask, cv2.IMREAD_GRAYSCALE)
        if sam_img is None:
            raise FileNotFoundError(f"無法讀取 SAM3 mask：{args.sam_mask}")
        # 正規化到 [0, 1]（白色=1=偵測區域，黑色=0=背景）
        sam_weight_map = sam_img.astype(np.float32) / 255.0
        print(f"✓ SAM3 mask 已載入：{args.sam_mask}，shape={sam_weight_map.shape}，"
              f"覆蓋率={( sam_weight_map > 0.5).mean() * 100:.1f}%")
    ...
    # ── Phase 2 Injection Schedule ──
    phase2_inject_schedule: Optional[list] = None
    if args.phase2_inject_weights.strip() and image_raw_features is not None:
        parsed_w2 = [float(w) for w in args.phase2_inject_weights.strip().split()]
        if len(parsed_w2) != total_scales:
            raise ValueError(
                f"--phase2_inject_weights 長度 ({len(parsed_w2)}) 與 scale 總數 ({total_scales}) 不符。"
            )
        phase2_inject_schedule = parsed_w2
        inj2_str = '  '.join([f"s{si}={w:.2f}" for si, w in enumerate(phase2_inject_schedule)])
        print(f"[Phase 2 Injection Schedule] {inj2_str}")
    ...
    if source_focus_token_indices and len(source_extractor.attention_maps) > 0:
        ...
        # SAM3 不加權 low_attn（preserve 區域不需要放大）
    ...
    if args.save_attn_vis:
        ...
        if p2p_token_storage.masks:
            combined_vis = {si: ~m[0, 0, :, :, 0].numpy() for si, m in p2p_token_storage.masks.items()}
            _save_masks_to_dir(
    ...
    with autocast(dtype=torch.bfloat16):
        with torch.no_grad():
            target_image = gen_one_img(
                ...
                # Phase 2 注入 source image feature（若有 phase2_inject_schedule）
                inject_image_features=image_raw_features if phase2_inject_schedule else None,
                inject_schedule=phase2_inject_schedule,
            )
