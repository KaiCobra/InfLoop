...
def compute_attention_mask_for_scale(
    ...
    filtered_attn, num_outliers, num_used = _iqr_filtered_mean(
        attn_stack,
        use_iqr_filter=use_iqr_filter,
    )

    # 二值化：z-score normalized threshold 或固定百分位數
    if use_normalized_attn:
        # z-score normalization: f(x) = (x - mean) / std
        # mask 區域 = f(x) > x（normalized 值大於原始值的位置）
        ...
        if low_attn:
            # preserve 區域：normalized < original（結構背景）
            region = normalized < filtered_attn
            coverage_pct = region.mean() * 100
            print(
                f"  Scale {scale_idx}: "
                f"使用 {num_used}/{num_used + num_outliers} 個 block，"
                f"normalized attn (low): μ={mu:.4f} σ={sigma:.4f}，"
                f"preserve 區域佔比 = {coverage_pct:.1f}%"
            )
        else:
            # focus 區域：normalized > original（高 attention 位置）
            region = normalized > filtered_attn
            coverage_pct = region.mean() * 100
            print(
                f"  Scale {scale_idx}: "
                f"使用 {num_used}/{num_used + num_outliers} 個 block，"
                f"normalized attn: μ={mu:.4f} σ={sigma:.4f}，"
...
def compute_attention_mask_dynamic_threshold(
    ...
    """
    使用 reference mask 引導，透過三分搜尋 (ternary search) 找到
    最大化 IoU(candidate_mask, target_mask) 的 attention percentile threshold。

    IoU 隨 percentile 先升後降（unimodal），三分搜尋每輪淘汰 1/3 搜尋區間。
    ...
    Returns:
        mask: (spatial_h, spatial_w) bool ndarray
            low_attn=False: True = focus（高 attention，不替換）
            low_attn=True:  True = preserve（低 attention，強制保留 source）
        None 若無可用 attention map
    """
    # ── Step 1: 收集 attention maps + IQR 過濾 ──
    all_block_attn_maps = []
    for block_idx in block_indices:
        attn_map = extractor.extract_word_attention(
            block_idx=block_idx,
    ...
    # ── Step 2: 若 ref_mask 不可用，fallback 到固定 percentile ──
    if ref_mask is None or ref_mask.sum() == 0 or ref_mask.all():
        if low_attn:
            thr = np.percentile(filtered_attn, 100.0 - fallback_percentile)
            region = filtered_attn < thr
        else:
            thr = np.percentile(filtered_attn, fallback_percentile)
            region = filtered_attn >= thr
        print(
            f"  Scale {scale_idx}: "
            f"{num_used}/{num_used + num_outliers} blocks，"
            f"ref_mask 不可用 → fallback percentile={fallback_percentile:.0f}"
        )
        return region.astype(bool)

    # ── Step 3: Resize filtered_attn 到 ref_mask 尺寸 ──
    ...
    # ── Step 4: Ternary search — maximize IoU ──
    ...
    # 在原始 scale 解析度套用
    if low_attn:
        region = filtered_attn < best_thr
    else:
        region = filtered_attn >= best_thr

    # 面積安全檢查
    coverage_pct = region.mean() * 100
    if coverage_pct < 1.0 or best_iou < 0.01:
        if low_attn:
            thr = np.percentile(filtered_attn, 100.0 - fallback_percentile)
            region = filtered_attn < thr
        else:
        ...
        print(
            f"  Scale {scale_idx}: "
            f"{num_used}/{num_used + num_outliers} blocks，"
            f"IoU search failed (IoU={best_iou:.3f}, cov={coverage_pct:.1f}%) "
            f"→ fallback percentile={fallback_percentile:.0f}"
        )
    else:
        ...
        print(
            f"  Scale {scale_idx}: "
            f"{num_used}/{num_used + num_outliers} blocks，"
            f"ternary search {mode_str}: pct={best_pct:.1f}，"
            f"thr={best_thr:.4f}，IoU={best_iou:.3f}，"
...
def _iqr_filtered_mean(
    ...
    若 use_iqr_filter=False，則直接對所有 block 取平均。
    ...
    if not use_iqr_filter:
        return attn_stack.mean(dim=0).numpy(), 0, num_blocks
...
def collect_attention_text_masks_dynamic(
    ...
    """
    使用 reference mask 引導的二分法搜尋閾值，收集各 scale 的 attention mask。
    ...
    """
    text_masks: Dict[int, np.ndarray] = {}
    mode_label = "低 attention preserve (dynamic)" if low_attn else "高 attention focus (dynamic)"
    print(f"\n[Dynamic Threshold – {label} ({mode_label})] scale {num_full_replace_scales} ~ {len(scale_schedule)-1}")
    ...
    print(f"[Dynamic Threshold – {label}] 收集到 {len(text_masks)} 個 scale 的 mask。")
    return text_masks
...
def add_common_arguments(parser):
    ...
    parser.add_argument('--use_iqr_filter', type=int, default=1, choices=[0, 1],
                        help='1=對 block attention 使用 IQR 過濾後再平均；0=直接對所有 block map 取平均')
...
if __name__ == '__main__':
    ...
    print(f"IQR filter         : {bool(args.use_iqr_filter)}")
    ...
    if source_focus_token_indices and len(source_extractor.attention_maps) > 0:
        source_text_masks = collect_attention_text_masks(
            ...
            label="source",
            low_attn=False,
            use_iqr_filter=bool(args.use_iqr_filter),
        )
